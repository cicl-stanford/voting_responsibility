...
```{r eval=F}
brm(formula = importance ~ 1 + pivotality + (1 + pivotality | participant),
    data = df.exp4.long)
```
...
```{r eval=F}
brm(formula = surprise ~ 1 + badness + (1 + badness | participant),
    data = df.exp4.long)
```
...
```{r eval=F}
brm(formula = responsibility ~ 1 + importance + surprise + (1 + importance + surprise | participant),
    data = df.exp4.long)
```
...
```{r eval=F}
brm(formula = responsibility ~ 1 + badness_dummy * (importance + surprise) + (1 + badness_dummy * (importance + surprise) | participant),
    data = df.exp4.long)
```
...
# wrongness model 
brm(formula = wrongness ~ 1 + importance + surprise + (1 + importance + surprise | participant),
    data = df.exp4.long)
```
...
```{r}
df.exp4 = read.csv("../../data/experiment_4/experiment_4.csv",
                   sep = ";",
                   fileEncoding = "UTF-8-BOM") %>% 
  clean_names() %>% 
...
# trial information 
df.exp4.trialinfo = read.csv("data/experiment4_scenarios.csv",
                             sep = ";",
                             fileEncoding = "UTF-8-BOM")
