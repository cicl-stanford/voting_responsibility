...
fun_scatterplot = function(data, x, y){
  ggplot(data = data,
...
df.exp4.regression = df.exp4.long %>% 
  pivot_wider(names_from = question,
              values_from = rating) %>% 
  group_by(participant) %>% 
  mutate(badness_dummy = ifelse(badness > median(badness), "bad", "neutral")) %>% 
  ungroup()
...
df.exp4.demographics = df.exp4 %>% 
  ...
  mutate(gender = tolower(gender),
         across(c(duration, age), ~ as.numeric(.)))
...
Perceived importance matters more for judgments of responsibility compared to wrongness judgments. 
...
- note participants who gave the same response for one of these questions across all trials are removed from this analysis

```{r}
df.exp4.regression.scaled = df.exp4.regression %>% 
  group_by(participant) %>% 
  mutate(across(.cols = c(importance, surprise, responsibility, wrongfulness),
                .fns = ~ scale(.))) %>% 
  ungroup()
...
fit.brm_exp4_responsibility_importance_surprise_scaled = brm(
  ...
  seed = 1,
  cores = 2,
  file = "cache/brm_exp4_responsibility_importance_surprise_scaled")
...
fit.brm_exp4_wrongfulness_importance_surprise_scaled = brm(
  ...
  iter = 8000,
  control = list(adapt_delta = 0.99),
  file = "cache/brm_exp4_wrongfulness_importance_surprise_scaled")
...
#### The role of surprise and importance for responsibility 

- compare a model with both surprise + importance vs. just importance 
- model that includes both predictors performs a little better (check about random effects structure)
...
fit.brm_exp4_responsibility_importance = add_criterion(
  ...
  criterion = "loo",
  reloo = T,
  file = "cache/brm_exp4_responsibility_importance")

fit.brm_exp4_responsibility_importance_surprise = add_criterion(
  fit.brm_exp4_responsibility_importance_surprise,
  criterion = "loo",
  reloo = T,
  file = "cache/brm_exp4_responsibility_importance_surprise")
...
df.plot = df.exp4.long %>% 
  mutate(pivotality = factor(pivotality,
                             levels = c(0.25, 1/3, 0.5, 1),
                             labels = c("1/4", "1/3", "1/2", "1"))) %>% 
  filter(question == "importance")

ggplot(data = df.plot,
       mapping = aes(x = pivotality,
                     y = rating)) + 
  stat_summary(fun.data = "mean_cl_boot",
               size = 0.5) +
...
ggsave("../../figures/plots/experiment4_pivotality_importance.pdf",
       width = 8,
       height = 6)
```
...
### 4. Modal badness interaction 

- split into surprise low vs. high
- 

```{r}
# df.data = df.exp4.long

ggpredict(fit.brm_exp4_responsibility_badness_dummy,
          terms = c("importance", "surprise", "badness_dummy"),
          condition = c(surprise = 10)) %>% 
  plot()
```
...
p1 = fun_scatterplot(data = df.plot,
                x = "importance",
                y = "responsibility")

p2 = fun_scatterplot(data = df.plot,
                x = "importance",
                y = "wrongfulness")
...
#### when controlling for surprise 

```{r}
df.plot = fit.brm_exp4_wrongfulness_importance_surprise %>% 
  fitted(newdata = df.exp4.regression.means,
         re_formula = NA) %>% 
  as_tibble() %>% 
  clean_names() %>%
